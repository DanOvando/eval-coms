...
SchaeferParallelSearch <-
  function(ni,
    ...
    # assign("schaefer",schaefer, envir = environment() )


    # assign("foo",foo, envir = environment() )
...
cpp <-
  foreach (
    i = 1:npoints,
    .combine = 'rbind',
    .packages = c('foreach',"here"),
    .inorder = TRUE,
    .export = "schaefer"
  ) %do% {
    # Rcpp::sourceCpp(here::here("src","schaefer.cpp"))
...
cpptime = Sys.time() - a
# browser()
# test <- inmemorytable[!is.na(inmemorytable[,1]),]

inmemorytable <- cpp[cpp[,1] != 0, ]
